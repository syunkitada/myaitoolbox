...
**「1 Agent = 1 Process」のアーキテクチャ**を採用し、特定の役割（クラス）を持たせたワーカープロセスとして常駐起動する。

起動例:
```bash
agentrun --watch --interval 10s --agent operator --name operator1
```
- `--agent`: 担当するAgentのクラス（例: `task-manager`, `operator`）
- `--name`: このプロセス（インスタンス）の固有識別子

複数インスタンスでの稼働（早い者勝ちによるTask分散）を前提とする。

役割

- Task監視（自身にアサインされたTaskの検知）
- Lock管理
- Agent起動とタイムアウト制御
- 成果物の最低限のバリデーション
- metadata更新
...
`task-manager` によるトリアージを含む、全体的なワークフローは以下のようになる。
...
```
 [External Event]
  ...
  +------------------+
        |
  (2) Watch (status: inbox, Assign: task-manager*)
        v
  +-------------------------+
  | agentrun (task-manager) |
  +-------------------------+
        |
  (3) Evaluate & Handoff
        v
  +------------------+
  | $WORKSPACE_ROOT/ |
  |      tasks/      |  <-- status: inbox, Assign: operator* に更新される
  +------------------+
        |
  (4) Watch (status: inbox, Assign: operator*)
        v
  +---------------------+
  | agentrun (operator) |
  +---------------------+
...
current_assignee: # 例: "operator*" (任意のoperator) または "operator1" (特定のインスタンス)
...
`agentcrawl` によって生成された新規Taskは、すべて `task-manager*` に初期アサインされる。`task-manager` エージェントはイベントの内容を解析し、適切なAgentへ処理を委譲する `handoff.md` を生成して終了する。

例1: 役割（クラス）を指定して、手の空いている誰かに委譲する場合
```yaml
next_assignee: developer*
reason: ソースコードの実装が必要であると判断したため
```

例2: 特定のインスタンス（以前のコンテキストを持つAgentなど）を指名する場合
```yaml
next_assignee: operator1
...
```
Task検知 (status: inbox 且つ current_assignee が自身の --name または --agent* にマッチ)
...
初期実装ではOpenCodeを利用する。
Runtimeは自身の `--agent` 引数に基づいて対象のディレクトリを決定し、以下を実行する。
...
- **Lock内容:** `.lock/owner.yaml` を作成し、以下を記録する。
  ```yaml
  pid: 12345
  name: operator1 # agentrunの起動時引数 --name
...
### agentrun
- `1 Agent = 1 Process` のワーカーモデル (`--agent`, `--name` 引数)
- Task監視 (ワイルドカードおよび直接指名によるRouting)
