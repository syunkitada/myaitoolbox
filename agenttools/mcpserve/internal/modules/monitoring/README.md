...
ダッシュボード系ツール（`list_dashboards`、`get_dashboard`、`create_dashboard`、`delete_dashboard`、`list_folders`）は `GRAFANA_URL` と `GRAFANA_API_TOKEN` が設定されている場合に有効で、`GRAFANA_DATASOURCE_UID` は不要です。`get_dashboard` の `verbose` は取得結果にダッシュボードの完全なモデルを含めるかどうかを切り替えます。
