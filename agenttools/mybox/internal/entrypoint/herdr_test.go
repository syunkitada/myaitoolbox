...
func TestHerdrStartTaskAgent(t *testing.T) {
	...
	assert.Equal(t, herdrTaskAgentName(task.ID), res.Agent.Name)
...
const herdrStartFileTabCreateJSON = `{"id":"cli:tab:create","result":{"type":"tab_created",` +
	`"tab":{"tab_id":"w7:t3","workspace_id":"w7","label":"20260919_foo"},` +
	`"root_pane":{"pane_id":"w7:p3","workspace_id":"w7"}}}`

const herdrStartFileAgentJSON = `{"id":"cli:agent:list","result":{"agents":[` +
	`{"agent":"opencode","name":"f20260919_foo","agent_status":"working","cwd":"/tmp/test",` +
...
func TestHerdrStartFileAgentReusesExistingAgent(t *testing.T) {
	...
	assert.Equal(t, "f20260919_foo", res.Agent.Name)
...
func TestHerdrStartFileAgentCreatesTabAndStarts(t *testing.T) {
	...
	assert.Equal(t, []string{"tab", "create", "--workspace", "w7", "--label", "20260919_foo", "--cwd", app.Project.Path}, calls[3])
	assert.Equal(t, []string{"agent", "start", "f20260919_foo", "--kind", "opencode", "--pane", "w7:p3"}, calls[4])
}
	...
	s.herdrRun = func(ctx context.Context, args ...string) ([]byte, error) {
		...
		case args[0] == "tab" && args[1] == "list":
			return []byte(`{"id":"cli:tab:list","result":{"tabs":[` +
				`{"label":"20260919_foo","number":1,"pane_count":1,"tab_id":"w7:t4","workspace_id":"w7"}]}}`), nil
...
func TestHerdrStartFileAgentBootstrapsWorkspace(t *testing.T) {
	...
	assert.Equal(t, []string{"tab", "rename", "w9:t1", "20260919_foo"}, calls[3])
	assert.Equal(t, []string{"agent", "start", "f20260919_foo", "--kind", "opencode", "--pane", "w9:p1"}, startArgs)
}
...
func TestHerdrStartFileAgentValidation(t *testing.T) {
	...
	for _, body := range []map[string]string{
		...
		{"path": "../escape.go"},
		{"path": "notes..md"},
		// Only files inside a task directory can start an agent.
		{"path": "src/app.go"},
		{"path": "README.md"},
		{"path": "notes/idea.md"},
		{"path": "tasks.md"},
...
func TestHerdrTaskAgentName(t *testing.T) {
	for _, tc := range []struct{ in, want string }{
		{"20260919_foo", "f20260919_foo"},
		{"foo", "foo"},
		{"Foo", "foo"},
		{"foo_bar", "foo_bar"},
		{"foo-bar", "foo-bar"},
		{"foo bar", "foo-bar"},
		{"20260919_My-Task", "f20260919_my-task"},
		{"UPPERCASE", "uppercase"},
		{"123", "f123"},
		{"!!!", ""},
		{"a-_-b", "a-_-b"},
		...
		tc := tc
		assert.Equal(t, tc.want, herdrTaskAgentName(tc.in), tc.in)
	}

	long := strings.Repeat("x", 40) + strings.Repeat("-y", 10)
	got := herdrTaskAgentName(long)
