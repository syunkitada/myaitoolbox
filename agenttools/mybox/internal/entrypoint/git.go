...
type gitDetail struct {
	...
	SyncStatus        string     `json:"sync_status"` // no-upstream, up-to-date, push, pull, diverged
...
func gitRepoDetail(dir string) gitDetail {
	...
	switch {
	case d.Upstream == "":
		d.SyncStatus = "no-upstream"
	case d.Ahead > 0 && d.Behind > 0:
		d.SyncStatus = "diverged"
	case d.Ahead > 0:
		d.SyncStatus = "push"
	case d.Behind > 0:
		d.SyncStatus = "pull"
...
func (s *Server) PostGitFetch(w http.ResponseWriter, r *http.Request) {
	...
	gitOpsMu.Lock()
	defer gitOpsMu.Unlock()
	out, err := runGit(dir, "fetch", "--all", "--prune")
	if err != nil {
		writeGitResult(w, http.StatusOK, gitResult{Ok: false, Output: out})
		return
	}
