...
// the agent bound to the task's directory (kind) and, when prompt is non-empty, render
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !isLoopbackHost(host) {
				_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "warning: mybox is listening on %s without authentication; do not expose it to untrusted networks\n", addr)
			}
