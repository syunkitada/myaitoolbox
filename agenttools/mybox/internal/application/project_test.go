...
func TestProjectRemoveSelectsNextDefault(t *testing.T) {
	first := t.TempDir()
	second := t.TempDir()
	store := &memoryConfigStore{cfg: &domain.Config{Projects: []domain.Project{
		{Name: filepath.Base(first), Path: first},
		{Name: filepath.Base(second), Path: second},
	}, DefaultProject: filepath.Base(first)}}
	uc := NewProjectUseCase(store)

	require.NoError(t, uc.Remove(context.Background(), filepath.Base(first)))
	cfg, _ := store.Load(context.Background())
	assert.Equal(t, filepath.Base(second), cfg.DefaultProject)
}
