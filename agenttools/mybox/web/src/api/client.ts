...
export interface GitDetail {
  ...
  remote: string
  upstream: string
  sync_status: 'no-upstream' | 'up-to-date' | 'push' | 'pull' | 'diverged' | string
  ahead: number
  behind: number
...
async function request<T>(
  ...
  const currentProject = projectOverride ?? getProject()
  if (currentProject) {
...
export const api = {
  ...
  listFiles: (opts?: { path?: string; showHidden?: boolean; project?: string }) => {
    const q: string[] = []
    if (opts?.path !== undefined && opts?.path !== null) q.push(`path=${encodeURIComponent(opts.path)}`)
    if (opts?.showHidden !== undefined) q.push(`show_hidden=${opts.showHidden}`)
  ...
  uploadFiles: (directory: string, files: File[]) => {
    ...
    return request<void>('POST', '/api/files/upload', form)
  },
  ...
  gitFetch: (scope?: string) =>
    request<GitResult>('POST', '/api/git/fetch' + qs({ path: scope })),
