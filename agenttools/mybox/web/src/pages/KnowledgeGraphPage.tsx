...
import { normalizeAlias } from '../graph/links'
...
function KnowledgeGraphBody() {
  ...
  const [knowledge, setKnowledge] = useState<Array<{ path: string; title?: string; aliases?: string[] | null }>>([])
  ...
  const linkContext = useMemo(() => {
    ...
    for (const k of knowledge) {
      if (k.title) aliases.set(normalizeAlias(k.title), k.path)
      for (const a of k.aliases ?? []) aliases.set(normalizeAlias(a), k.path)
    }
    return { knownMarkdown, aliases }
  }, [tree, knowledge])
