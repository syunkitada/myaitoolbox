...
describe('parseFrontmatter', () => {
  ...
  it('reports the parse error message for invalid yaml', () => {
    const parsed = parseFrontmatter('description: value with Examples:\n<example>\nContext: nope\n</example>')
    expect(parsed.ok).toBe(false)
    expect(parsed.error).toMatch(/[Mm]apping/)
  })
})
...
describe('setMarkdownTaskChecked', () => {
  it('updates unchecked and checked task markers by line', () => {
    const text = '- [ ] first\n- [x] second\n- normal'
    expect(setMarkdownTaskChecked(text, 0, true)).toBe('- [x] first\n- [x] second\n- normal')
    expect(setMarkdownTaskChecked(text, 1, false)).toBe('- [ ] first\n- [ ] second\n- normal')
  })

  it('supports nested and ordered task lists and preserves CRLF', () => {
    const text = '1. [ ] first\r\n  - [X] nested\r\n'
    expect(setMarkdownTaskChecked(text, 1, false)).toBe('1. [ ] first\r\n  - [ ] nested\r\n')
  })

  it('returns null for a non-task line', () => {
    expect(setMarkdownTaskChecked('- normal', 0, true)).toBeNull()
    expect(setMarkdownTaskChecked('- [ ] task', 3, true)).toBeNull()
  })
})

describe('replaceMarkdownBody', () => {
  it('preserves the original frontmatter formatting', () => {
    const text = '---\ntitle:  X # keep this\n---\n\n- [ ] task\n'
    expect(replaceMarkdownBody(text, '- [x] task\n')).toBe(
      '---\ntitle:  X # keep this\n---\n\n- [x] task\n',
    )
  })

  it('returns body unchanged when frontmatter is absent', () => {
    expect(replaceMarkdownBody('# Title', '- [ ] task')).toBe('- [ ] task')
  })
})

describe('markdown copy formats', () => {
  it('keeps text as the default copy format', () => {
    const text = '# Title\n\nplain text'
    expect(formatMarkdownForCopy(text)).toBe(text)
    expect(formatMarkdownForCopy(text, 'text')).toBe(text)
  })

  it('converts common Markdown formatting to Jira wiki markup', () => {
    const text = '# Title\n\n**bold** and _italic_ with [link](https://example.com) and `code`.\n\n- first\n  - nested\n\n```ts\nconst value = 1\n```'
    expect(markdownToJira(text)).toBe(
      'h1. Title\n\n*bold* and _italic_ with [link|https://example.com] and {{code}}.\n\n* first\n** nested\n\n{code:ts}\nconst value = 1\n{code}',
    )
  })

  it('converts Jira tables and preserves a trailing newline', () => {
    const text = '| Name | Value |\n| --- | --- |\n| A | 1 |\n'
    expect(markdownToJira(text)).toBe('||Name||Value||\n|A|1|\n')
  })
})
